...
init() {
  # copy sql
  echo "begin copy sql "
  cp ../data/gins.sql ./mysql/db
  cp ../data/gins_job.sql ./mysql/db

  # copy jar
  echo "begin copy gins-admin "
  cp ../gins-admin/target/gins-admin.jar ./gins/gins-admin/jar
}
...
# 启动程序模块（必须）
server() {
  docker-compose up -d gins-admin
}
